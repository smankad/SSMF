...
ssnmf_wglmnet = function(Y,
                    X,
                    numComp = 1,
                    maxiter = 500,
                    tries = 1) {
  
  # single stage factorization with a lasso penalty when solving for the regression coefficients
  #
  # Args:
  ...
  solveBeta = function(Y, Lambda, X) {
    ...
    if (length(Y) <= 200 ){
        cv_glmnet <- cv.glmnet(y=Y, x = XL, intercept = TRUE, lower.limits = -1, upper.limits = 1, nfolds=length(Y))
    } else {
        cv_glmnet <- cv.glmnet(y=Y, x = XL, intercept = TRUE, lower.limits = -1, upper.limits = 1, nfolds=5)
    }
    fit <- glmnet(y=Y, x=XL, intercept = TRUE, lambda = cv_glmnet$lambda.min, lower.limits = -1, upper.limits = 1)
    coef <- fit$beta
